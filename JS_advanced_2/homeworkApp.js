...
addButtons.forEach(button => {
    button.addEventListener('click', (e) => {
        if (e.target.id === 'apple' || e.target.id === 'sony' || e.target.id === 'samsung') {
            newReviewRender(e);
        }
...
function newReviewRender(e) {
    try {
        ...
        if (textArea.value.length < 50) throw new Error('Напиши побольше, что тебе жалко, чтоли');
        if (textArea.value.length > 500) throw new Error('Ну ты размахнулся, удали чуток');
